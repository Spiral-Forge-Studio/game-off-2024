...
public class RocketCritRateBuff : Buff
{
    public BuffType buffType;
    public Rarity rarity;
    public string buffname = "Rocket Crit Rate Buff";
    ...
    public RocketCritRateBuff(PlayerStatusSO status, float duration, BuffType buffType, Rarity rarity, float initialAmount, float consecutiveAmount, float scalingFactor)
        : base(duration)
    {
    ...
    public override void ApplyBuff(GameObject target)
    {
        if (buffType == BuffType.Flat)
        {
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.RocketCritRateFlatBonus, initialAmountFlat);
    ...
    public override void ApplyConsecutiveBuff()
    {
        if (buffType == BuffType.Flat)
        {
            ...
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.RocketCritRateFlatBonus, reductionAmount);
    ...
    public override void RemoveBuff(GameObject target)
    {
        if (buffType == BuffType.Flat)
        {
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.RocketCritRateFlatBonus, -totalFlatReduction);
        }
        if (buffType == BuffType.Percentage)
        {
