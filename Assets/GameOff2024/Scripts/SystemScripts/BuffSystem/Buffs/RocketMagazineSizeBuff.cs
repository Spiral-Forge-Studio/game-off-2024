...
public class RocketMagazineBuff : Buff
{
    ...
    public RocketMagazineBuff(PlayerStatusSO status, float duration, BuffType buffType, Rarity rarity, float initialAmount, float consecutiveAmount, float scalingFactor)
        : base(duration)
    {
    ...
    public override void RemoveBuff(GameObject target)
    {
        ...
        if (buffType == BuffType.Percentage)
        {
            playerStatus.ModifyMultiplier(EStatTypeMultiplier.RocketMagazineSizeMultiplier, -totalMultiplier * 100f, false);
        }
