...
public class RocketCritDamageBuff : Buff
{
    ...
    public RocketCritDamageBuff(PlayerStatusSO status, float duration, BuffType buffType, Rarity rarity, float initialAmount, float consecutiveAmount, float scalingFactor)
        : base(duration)
    {
    ...
    public override void RemoveBuff(GameObject target)
    {
        ...
        if (buffType == BuffType.Percentage)
        {
            playerStatus.ModifyMultiplier(EStatTypeMultiplier.RocketCritDamageMultiplier, -totalMultiplierReduction * 100f, false);
        }
