...
public class MinigunCritRateBuff : Buff
{
    ...
    public MinigunCritRateBuff(PlayerStatusSO status, float duration, BuffType buffType, Rarity rarity, float initialAmount, float consecutiveAmount, float scalingFactor)
        : base(duration)
    {
    ...
    public override void ApplyBuff(GameObject target)
    {
        if (buffType == BuffType.Flat)
        {
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.MinigunCritRateFlatBonus, initialAmountFlat);
    ...
    public override void ApplyConsecutiveBuff()
    {
        if (buffType == BuffType.Flat)
        {
            ...
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.MinigunCritRateFlatBonus, reductionAmount);
            totalFlatReduction += reductionAmount;
        }
        if (buffType == BuffType.Percentage)
        {
            float reductionAmount = consecutiveAmountMultiplier * (1 + scalingFactor);
            playerStatus.ModifyMultiplier(EStatTypeMultiplier.MinigunCritRateMultiplier, reductionAmount, true);
    ...
    public override void RemoveBuff(GameObject target)
    {
        if (buffType == BuffType.Flat)
        {
            playerStatus.ModifyFlatBonus(EStatTypeFlatBonus.MinigunCritRateFlatBonus, -totalFlatReduction);
        }
        if (buffType == BuffType.Percentage)
        {
            playerStatus.ModifyMultiplier(EStatTypeMultiplier.MinigunCritRateMultiplier, -totalMultiplierReduction * 100f, false);
        }
